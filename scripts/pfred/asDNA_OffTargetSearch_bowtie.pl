...
# created: Qing Cao
# date:  Sept 28, 2009
# usage: asDNA_OffTargetSearch_bowtie -t inputType -s species -g GeneEnsemblID seqs.txt
...
my @BOWTIE_SearchList= & exportAntisenseIndexes("human");
#my @BOWTIE_SearchList=("HumancDNA.v56", "allHumanUnsplicedGene.v55");
my $BOWTIE_option="-a";
...
if($opts{'s'} =~ /mouse/i){
    #push @BOWTIE_SearchList,"MousecDNA.v55","allMouseUnsplicedGene.v55";
    push @BOWTIE_SearchList, & exportAntisenseIndexes("mouse");
}
if($opts{'s'}=~ /rat/i){
...
for (my $i=0; $i< @BOWTIE_SearchList; $i++){
    ...
    my $type=($db=~ /cDNA/i)? "cDNA":"unspliced";
    my $target=$opts{'g'};
    ...
    #------parse bowtie out ------------#
    $count{$db} = &parseBowtieOutput ($bowtie_out,$target,$type);
    for (my $j=0;$j<=$mismatch;$j++){
        print "\t${db}_${j}mismatch";
    }
    # print "\t${db}_OffTargetWith0mismatch\t${db}_OffTargetWit1mismatch";
}
print "\n";
...
foreach $name (@names){
    ...
    for (my $i=0;$i<@BOWTIE_SearchList;$i++){
        ...
        #print "\t$count{$db}->{$name}->[0]\t$count{$db}->{$name}->[1]";
    }
    print "\n";
